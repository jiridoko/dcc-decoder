...
def query():
    ...
    #d.get_controller().send(loco_functions_1(address=47, f2=True))
    #time.sleep(0.05)
    #d.get_controller().send(loco_functions_1(address=47))
    return ('', 204)

@app.route('/slider', methods=["POST"])
def slider():
    print("value: "+request.args['value'])
    #d.get_controller().send(loco_speed(address=47, speed=int(request.args['value'])))
    return ('', 204)
